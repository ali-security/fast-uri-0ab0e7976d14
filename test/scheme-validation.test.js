...
test('decoded schemes select their scheme handlers', (t) => {
  ...
  // `mailto` is not supported by fastifyURI, so the scheme-specific handler
  // lookup is asserted here with the equivalent `urn:uuid` handler.
  const urnuuid = fastURI.parse('%75rn:uuid:f81d4fae-7dec-11d0-a765-00a0c91e6bf6')
  t.equal(urnuuid.error, undefined, 'urn:uuid parses without error')
  t.equal(urnuuid.scheme, 'urn', 'the scheme is decoded')
  t.equal(urnuuid.nid, 'uuid', 'urn parsing runs after decoding the scheme')
  t.equal(urnuuid.uuid, 'f81d4fae-7dec-11d0-a765-00a0c91e6bf6', 'urn:uuid parsing runs after decoding the scheme')
  t.end()
})
